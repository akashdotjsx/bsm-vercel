// Script to create test tickets using the API
const testTickets = [
  {
  ...
  },
  {
    ...
    custom_fields: {
      ...
      "notes": "Customer reported via support chat"
    }
  },
  {
    ...
    custom_fields: {
      ...
      "notes": "Customer reported via phone"
    }
  }
]

async function createTestTickets() {
  console.log('Creating test tickets via API...')
  ...
  for (let i = 0; i < testTickets.length; i++) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ Created ticket: ${result.ticket?.title}`)
      } else {
