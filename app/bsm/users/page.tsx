...
export default function UsersPage() {
  ...
  const fetchDepartmentsAndRoles = async () => {
    try {
      ...
      if (deptError) {
        ...
        setDepartments(["Information Technology", "Human Resources", "Finance", "Customer Support", "Operations"])
      } else {
      ...
      try {
        ...
        if (rolesError) {
          ...
          // Use fallback roles that match the actual database schema
          setRoles([
        ...
        console.error("Roles table might not exist:", rolesFetchError)
        // Use fallback roles that match the actual database schema
        setRoles([
          "System Administrator",
          "Admin",
  ...
  const handleDeleteDepartment = async (dept) => {
    try {
      const { error } = await supabase.from("departments").delete().eq("name", dept)
      ...
      >
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Loading users and teams...</p>
          </div>
        </div>
      </PlatformLayout>
    )
