...
export const LOWER_CASE_REGEX = RegExp(/[a-z]/);
export const UPPER_CASE_REGEX = RegExp(/[A-Z]/);
export const NUMBER_REGEX = RegExp(/[0-9]/);
export const SYMBOL_REGEX = RegExp(/[!@#$%^&*()_+|*{}<>]/);
export const MINIMUM_PASSWORD_LENGTH = 8;
export const MAXIMUM_PASSWORD_LENGTH = 16;
export const MOBILE_REGEX = RegExp(/^[0-9\b]*$/);
export const UPI_REGEX = RegExp(/^[a-zA-Z0-9.-]{2, 256}@[a-zA-Z][a-zA-Z]{2, 64}$/);

export const FACEBOOK_URL = RegExp(
  /(?:(?:http|https):\/\/)?(?:www\.)?(?:facebook\.com)\/([A-Za-z0-9-_\.]+)/im,
);
export const LINKEDIN_URL = RegExp(
  /^(http(s)?:\/\/)?([\w]+\.)?linkedin\.com\/(pub|in|profile|company)\/([A-Za-z0-9-_\.]+)/im,
);
export const INSTAGRAM_URL = RegExp(
  /(?:(?:http|https):\/\/)?(?:www\.)?(?:instagram\.com|instagr\.am)\/([A-Za-z0-9-_\.]+)/im,
);

export const ESI_NUMBER = RegExp(
  /^(\d{2})[-–\s]?(\d{2})[-–\s]?(\d{1,6})[-–\s]?(\d{3})[-–\s]?(\d{4})$/,
);
export const PF_NUMBER = RegExp(
  /^([A-Z]{2}\s)([A-Z]{3}\s)([0-9]{1,7}\s)([0-9]{3}\s)?([0-9]{1,7})$/,
);
export const AMOUNT_REGEX = RegExp(/^\d+(\.\d{1,2})?$/);

export const ALPHA_NUMERIC_SPACE_REGEX = RegExp(/^[a-zA-Z0-9 ]*$/);
